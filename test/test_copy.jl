...
type AbstractSolutionData_ <: AbstractSolutionData{Float64, Float64}
  q::Array{Float64,2}
...
@testset "--- Testing copy_array_recursive ---" begin

  # test single level
  arr = rand(5)
  arr_dest = zeros(5)
  ODLCommonTools.copy_array_recursive!(arr_dest, arr)
  @test isapprox( norm(arr_dest - arr), 0.0) atol=1e-14
  ...
  ODLCommonTools.copy_array_recursive!(arr_dest, arr)
  for i=1:2
    @test isapprox( norm(arr_dest[i] - arr[i]), 0.0) atol=1e-14
  end
  ...
  for i=1:2
    for j=1:2
      @test isapprox( norm(arr_dest[i][j] - arr[i][j]), 0.0) atol=1e-14
    end
  end

end

@testset "--- Testing copy functions ---" begin
...
#   println(test_data_rand.res)
  ...
  @test isapprox( test_data.q, test_data_rand.q) atol=1e-14
  @test isapprox( test_data.q_vec, test_data_rand.q_vec) atol=1e-14
  @test isapprox( test_data.res, test_data_rand.res) atol=1e-14
  @test isapprox( test_data.res_vec, test_data_rand.res_vec) atol=1e-14
  for i = 1:num_inner_arrays
    @test isapprox( test_data.shared_data[i], test_data_rand.shared_data[i]) atol=1e-14
#    @test isapprox( test_data.q_face_send[i], test_data_rand.q_face_send[i]) atol=1e-14#    @fact test_data.q_face_send[i] --> roughly(test_data_rand.q_face_send[i], atol=1e-14)
#    @test isapprox( test_data.q_face_recv[i], test_data_rand.q_face_recv[i]) atol=1e-14#    @fact test_data.q_face_recv[i] --> roughly(test_data_rand.q_face_recv[i], atol=1e-14)
  end

  #---------------
  # testing that eqn_dest = copy(eqn_src::AbstractSolutionData) throws an error
  @test_throws Exception  test_data_dest = copy(test_data)
...
#   println(test_data_rand.unhandled_field)
  @test not_isapprox( test_data.unhandled_field, test_data_rand.unhandled_field) atol=1e-14

  # verfy no aliasing
  @test ( pointer(test_data.q) )!= pointer(test_data_rand.q) 
  @test ( pointer(test_data.shared_data) )!= pointer(test_data_rand.q) 
  for i=1:length(test_data.shared_data)
    @test ( pointer(test_data.shared_data[i]) )!= pointer(test_data_rand.shared_data[i])
  end
#   println(test_data)
